...
class VectorStoreManager:
    """A class to manage Qdrant collections, index tracking, and vector uploads for hybrid search."""
    ...
    def create_collection(self):
        """Creates the Qdrant collection configured for both dense and sparse vectors."""
        collections = self.client.get_collections()
        existing = [c.name for c in collections.collections]

        if self.collection_name in existing:
            logger.info(f"{self.collection_name} already exists")
            return
        ...
        logger.info(f"{self.collection_name} created for Hybrid Search")
    ...
    def store_vectors(self):
        """Indexes split document chunks and uploads hybrid representations to Qdrant."""
        self.create_collection()

        skip_hashes = self.get_indexed_hashes()
        if skip_hashes:
...
create_collection = _manager.create_collection
get_indexed_hashes = _manager.get_indexed_hashes
store_vectors = _manager.store_vectors
